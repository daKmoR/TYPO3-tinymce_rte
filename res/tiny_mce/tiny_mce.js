...
break;}
return s;}});(function(){var each=tinymce.each;tinymce.create('tinymce.util.URI',{URI:function(u,s){var t=this,o,a,b;u=tinymce.trim(u);s=t.settings=s||{};if(/^(mailto|tel|news|javascript|about):/i.test(u)||/^\s*#/.test(u)){t.source=u;return;}
...
return e;},select:function(pa,s){var t=this;return tinymce.dom.Sizzle(pa,t.get(s)||t.get(t.settings.root_element)||t.doc,[]);},is:function(n,patt){return tinymce.dom.Sizzle.matches(patt,n.nodeType?[n]:n).length>0;},add:function(p,n,a,h,c){var t=this;return this.run(p,function(p){var e,k;e=is(n,'string')?t.doc.createElement(n):n;t.setAttribs(e,a);if(h){if(h.nodeType)
...
h=h.replace(/<a( )([^>]+)\/>|<a\/>/gi,'<a$1$2></a>');if(s.keep_values){if(/<script|noscript|style/.test(h)){function trim(s){s=s.replace(/(<!--\[CDATA\[|\]\]-->)/g,'\n');s=s.replace(/^[\r\n]*|[\r\n]*$/g,'');s=s.replace(/^\s*(\/\/\s*<!--|\/\/\s*<!\[CDATA\[|<!--|<!\[CDATA\[)[\r\n]*/g,'');s=s.replace(/\s*(\/\/\s*\]\]>|\/\/\s*-->|\]\]>|-->|\]\]-->)\s*$/g,'');return s;};h=h.replace(/<script([^>]+|)>([\s\S]*?)<\/script>/g,function(v,attribs,text){if(!attribs)
attribs=' type="text/javascript"';attribs=attribs.replace(/(type|language)=\"?/,'$&mce-');attribs=attribs.replace(/src=\"([^\"]+)\"?/,function(a,url){if(s.url_converter)
...
text='<!--\n'+trim(text)+'\n// -->';return'<mce:script'+attribs+'>'+text+'</mce:script>';});h=h.replace(/<style([^>]+|)>([\s\S]*?)<\/style>/g,function(v,attribs,text){if(text)
...
t.events=new tinymce.dom.EventUtils();return t.events.remove(target,name,func);},_isRes:function(c){return/^(top|left|bottom|right|width|height)/i.test(c)||/;\s*(top|left|bottom|right|width|height)/i.test(c);}
...
np.appendChild(n);}}}});}
if(s.fix_table_elements){t.onPreProcess.add(function(se,o){each(t.dom.select('p table',o.node),function(n){if(isIE)
n.outerHTML=n.outerHTML;t.dom.split(t.dom.getParent(n,'p'),n);});});}},setEntities:function(s){var t=this,a,i,l={},re='',v;if(t.entityLookup)
...
return wa[i];}
return null;},serialize:function(n,o){var h,t=this,doc;t._setup();o=o||{};o.format=o.format||'html';n=n.cloneNode(true);t.processObj=o;if(tinymce.isWebKit){doc=n.ownerDocument.implementation.createHTMLDocument("");doc.body.appendChild(n);}
...
h=t._unprotect(h,p);h=h.replace(/<!--\[CDATA\[([\s\S]+)\]\]-->/g,'<![CDATA[$1]]>');h=h.replace(/(type|language)=\"mce-/g,'$1="');if(s.entity_encoding=='raw')
...
new tinymce.Editor(v,t.settings).render();return true;case"mceAddFrameControl":w=v.window;w.tinyMCE=tinyMCE;w.tinymce=tinymce;tinymce.DOM.doc=w.document;tinymce.DOM.win=w;if(tinymce.DOM.files&&v.reAddCss)
tinymce.DOM.files={};ed=new tinymce.Editor(v.element_id,v);ed.render();if(tinymce.isIE){function clr(){ed.destroy();w.detachEvent('onunload',clr);w=w.tinyMCE=w.tinymce=null;};w.attachEvent('onunload',clr);}
